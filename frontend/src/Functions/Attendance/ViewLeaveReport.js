...
function ViewLeaveReport() {
  ...
  async function onSubmit(event) {
    ...
    try{
      await axios.get(`http://localhost:4000/attendance/leave/get/${name._id}` )
          .then(response => {
            // console.log(JSON.stringify(response.data));
            const data = response.data;
            setrows([]);
  ...
  const PdfGen = () =>(
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <Text style={styles.title}>Detailed Mark Leave Report</Text>
      ...
      <div>
        <div style={{ height: 690, width: 800, margin: '1% 2% 2% 2%', background:'#cecece6b'}}>
          <Stack spacing={2} sx={{ m: 4,mx: "auto" }}>
            ...
            <div>
              <PDFDownloadLink document={<PdfGen />} fileName="Mark-Leave-Report.pdf" style={{
                borderRadius: 30,
